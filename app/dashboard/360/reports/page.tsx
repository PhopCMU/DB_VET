...
export default function Page() {
  ...
  const handleExportExcel = async () => {
    ...
    selectedUserList.forEach((user: any) => {
      ...
      const evaluationData = user.evaluations.map(
    ...
    exportMultipleSheetsToExcel(exportData, "รายงาน_360.xlsx");
  };

  // Calculate average score from all evaluations
  const calculateAverageScore = (evaluations: any) => {
    if (selectedOrg.id === "3") {
      const total = evaluations.reduce((sum: number, evaluation: any) => {
        return (
          sum +
          (evaluation.topic1 +
            evaluation.topic2 +
  ...
  const findMaxScore = (evaluations: any) => {
    const scores =
      evaluations &&
      evaluations.map((evaluation: any) => calculateTotalScore(evaluation));
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm font-medium flex items-center">
                    ...
                    {selectedUser.evaluations.length} ครั้ง
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-blue-50 p-3 rounded-lg">
                    ...
                    <div className="text-2xl font-bold text-blue-800 mt-1">
                      {calculateAverageScore(selectedUser.evaluations).toFixed(
                        1,
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-green-50 p-3 rounded-lg">
                    ...
                    <div className="text-sm font-medium text-green-800 mt-1">
                      {selectedUser.evaluations?.length > 0 ? (
                        <div className="text-sm font-medium text-green-800 mt-1">
                          {formatDate(
                            selectedUser.evaluations.at(-1).updatedAt,
                          )}
                        </div>
                      ) : (
                  ...
                  <div className="bg-purple-50 p-3 rounded-lg">
                    ...
                    <div className="text-2xl font-bold text-purple-800 mt-1">
                      {findMaxScore(selectedUser.evaluations)}
                    </div>
                  </div>
                </div>
              </motion.div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mt-3">
                        ...
                        {evaluation.topic7 && (
                          <div className="p-2 border border-emerald-100 rounded-lg">
                            ...
                            <div className="flex items-center justify-between mt-1">
                              <span className="text-[12px] text-gray-500 font-medium">
                                การตรวจสอบความถูกต้องตามกรระบวน
                              </span>
                ...
                <div className="space-y-3">
                  ...
                  <div className="divide-y divide-gray-200 border border-gray-200 rounded-lg overflow-hidden bg-white">
                    {filteredUsers?.map((user: any, index: number) => {
                      ...
                      const hasEvaluation = user.evaluations?.length > 0;
