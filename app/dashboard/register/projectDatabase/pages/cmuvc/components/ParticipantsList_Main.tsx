...
export default function ParticipantsList_Main() {
  ...
  const fetchDataParticipantsMain = async (date: Date) => {
  ...
  useEffect(() => {
    if (!hasData.current) {
      fetchDataParticipantsMain(new Date());
  ...
  const handerChangeYear = async (date: Date) => {
    ...
    await fetchDataParticipantsMain(date || new Date());
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await fetchDataParticipantsMain(selectedYear || new Date());
      setIsLoading(false);
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              <h1 className="text-2xl font-bold text-gray-800 mb-2 flex items-center gap-2">
                Names of Participants in the Main Conference
                <motion.span
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
