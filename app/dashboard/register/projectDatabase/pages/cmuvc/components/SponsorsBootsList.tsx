...
import { Delete_boot_Sponsor } from "@/app/routers/cmuvc/DeleteRouter";
import { GetBoots, GetSelector } from "@/app/routers/cmuvc/GetRouter";
import { ConfirmModal } from "@/components/ConfirmModal/ConfirmModal";
import PermissionGuard from "@/components/Guards/PermissionGuard";
import Loading from "@/components/Loadings/Loading";
import { LoadingModal } from "@/components/Modal";
import { ModalCreateSponsor } from "@/components/ModalCreated/Sponsor";
...
export default function SponsorsBootsList() {
  const { loading } = useUser();
  const [sponsorsBoots, setSponsorsBoots] = useState<any[]>([]);
  ...
  const filtered = useMemo(() => {
    if (!sponsorsBoots) return [];
    return sponsorsBoots.filter((p: any) => {
      ...
      return (
        ...
        p.companys.name.toLowerCase().includes(lowerSearchTerm)
      );
    });
  }, [sponsorsBoots, searchTerm]);
  ...
  useEffect(() => {
    ...
    if (visitorId && selectedYear) {
      debounceTimer.current = setTimeout(() => {
        fetchData(new Date() || selectedYear, visitorId); // Make sure to use the correct date
  ...
  const handerChangeYear = async (date: Date) => {
    ...
    await fetchData(date || new Date(), visitorId);
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await fetchData(selectedYear || new Date(), visitorId);
    ...
    <>
      ...
      <div className="min-h-screen">
        <motion.div
          ...
          className="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 hover:shadow-xl  hover:border-blue-100 hover:bg-gradient-to-br hover:from-blue-50/30 hover:to-white group backdrop-blur-sm"
        >
          <div className="flex items-start gap-4">
            {/* Icon Section */}
            <div className="p-3 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl text-blue-600 group-hover:from-blue-100 group-hover:to-blue-200  shadow-sm">
            ...
            <div className="flex-1">
              <h1 className="text-2xl font-bold text-gray-800 mb-2 flex items-center gap-3">
                ...
                <motion.span
                  ...
                  className="text-xs font-semibold px-3 py-1.5 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-full shadow-sm"
                >
                  {selectedYear ? selectedYear.getFullYear() + 543 : ""}
                </motion.span>
              </h1>
              <p className="text-sm text-gray-600 mb-4 leading-relaxed">
                Review of the Company Representative List
              </p>
            </div>
          </div>
        </motion.div>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 p-4 bg-white rounded-2xl shadow-sm border border-gray-100">
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-2 text-gray-600">
                      <p className="text-[12px]">
                        จำนวนผู้ที่ได้รับสนับสนุนทั้งหมด{" "}
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                >
                  ...
                  <input
                    type="text"
                    placeholder="ค้นหาชื่อ นามสกุล หรือ บริษัท..."
                    value={searchTerm}
                  ...
                  >
                    <tr>
                      {[
                        ...
                        >
                          <div
                            className={`flex items-center gap-2 ${header.point} group-hover:border-gray-300`}
                          >
                            {/* Icon with modern styling */}
                            <span className="material-symbols-outlined text-base text-blue-600 bg-blue-50 p-1.5 rounded-md">
                              {header.icon}
                            </span>
                          ...
                          <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-700 ">
                            <div className="flex flex-col items-start">
                              ...
                              <div className="text-[12px] text-gray-500 flex items-center mt-1 group relative">
                                ...
                                <span className="truncate max-w-[180px]">
                                  {user?.categorys?.categoryType
                                    ? (user.categorys?.categoryType).length > 20
                                      ? `${(user.categorys?.categoryType).substring(
                                          0,
                                          20,
                                        )}...`
                                      : user.categorys?.categoryType
