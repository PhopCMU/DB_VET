# Security: Dependencies & Supply Chain (Next.js)
...
5) Next.js build considerations:
   - Avoid adding heavy client-side deps unnecessarily (bundle size)
   - Ensure browser-incompatible packages are not imported by Client Components
...
- Pulling server-only packages into client bundles
