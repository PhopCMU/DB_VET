# Security: Auth with Axios (Token Storage + Cookies) in Next.js

## When to use
- Any authenticated request
- Implementing login/logout, session restore, refresh behavior
- Designing how frontend attaches credentials
- Deciding whether auth logic runs in Client Components or on the server
...
## Next.js notes
- Prefer doing privileged auth/session operations on the server when possible:
  - Route Handlers (e.g., `src/app/api/**/route.ts`) or root `app/api/**/route.ts`
  - Server Actions
- Client Components should not receive secrets (tokens, backend credentials) via props.
