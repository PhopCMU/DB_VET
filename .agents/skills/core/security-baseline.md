# Security Baseline (Next.js Frontend)

## When to use
- Always. This is the default security posture for this project.

## Threat model (Next.js frontend)
...
- Server/Client boundary mistakes (leaking secrets into Client Components)

## Next.js architecture notes (app router)
- Prefer `src/app/**` as the default root.
- Some projects may use root `app/**`. All rules in this repo must apply to both:
  - `src/app/**`
  - `app/**`
- Treat anything that runs in the browser as untrusted.
- Do not pass secrets from Server Components to Client Components.
...
8) Client/Server separation:
   - Keep secrets and privileged logic on the server (Route Handlers / Server Actions / Server Components)
...
- [ ] No secrets passed to Client Components; only `NEXT_PUBLIC_*` exposed
